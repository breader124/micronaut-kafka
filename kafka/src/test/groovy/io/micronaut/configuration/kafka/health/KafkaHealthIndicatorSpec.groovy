...
class KafkaHealthIndicatorSpec extends Specification {
    ...
    void "test kafka health indicator - UP"() {
        given:
        KafkaContainer kafkaContainer = new KafkaContainer(DockerImageName.parse("confluentinc/cp-kafka:7.0.4.arm64"))
    ...
    void "test kafka health indicator - disabled (#configvalue)"() {
        given:
        KafkaContainer container = new KafkaContainer(DockerImageName.parse("confluentinc/cp-kafka:7.0.4.arm64"))
        container.start()
        ApplicationContext applicationContext = ApplicationContext.run(
